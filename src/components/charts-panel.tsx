...
import type { CalcInputs } from "@/lib/landed-cost";
import { landedCost } from "@/lib/landed-cost";
...
export function ImportTrendChart({ analysis }: { analysis: AnalysisResult }) {
  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-slate-950/70 p-5 shadow-xl shadow-black/20">
      ...
      <div className="h-80 w-full" dir="ltr" role="img" aria-label={t("volumeTitle")}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={analysis.importTrend} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
            <defs>
...
export function CostBreakdownChart({ inputs }: { inputs: CalcInputs }) {
  const { t, locale } = useLanguage();
  const result = landedCost(inputs);
  const purchaseIls = inputs.purchaseUsd * inputs.usdIls;
  const freightIls = inputs.freightUsd * inputs.usdIls;

  const breakdown = [
    { key: "purchase", name: t("purchase"), value: Math.max(purchaseIls, 0), fill: "#3b82f6" },
    { key: "freight", name: t("freight"), value: Math.max(freightIls, 0), fill: "#38bdf8" },
  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-slate-950/70 p-5 shadow-xl shadow-black/20">
      ...
      <div className="h-72 w-full" dir="ltr" role="img" aria-label={t("costTitle")}>
        <ResponsiveContainer width="100%" height="100%">
